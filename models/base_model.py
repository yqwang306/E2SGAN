...
class BaseModel(ABC):
    ...
    def visualize_mid_layer_output(self, index, name, n_block, f_intv, save_dir):
        '''Save a visualization of the output of the middle layer of the model.
        
        :param index: the file index, which is used for naming
        :param name: the name of the subnet to be visualized
        :param n_block: the number of blocks to be output in the model
        :param f_intv: the output interval of the filter of the last CNN layer
        ...
        for i in range(n_block):
            ...
            conv_out = networks.LayerActivations(getattr(net, 'up_' + str(i)))  # modify the output module name according to your need
            net.test()
            conv_out.remove()  #
            act = conv_out.features

            for j in range(0, len(act[0]), f_intv):  # output results at every f_intv filters interval

                tmp = act[0][j].detach().numpy()
                h, w = tmp.shape
                name = 'up' + str(index) + '_' + str(i) + '_' + str(j)  # the image name to be saved
