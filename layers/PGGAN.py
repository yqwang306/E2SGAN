...
class HelpFunc(object):
    ...
    def process_transition(a, b):
        ...
        a_batch, a_channel, a_height, a_width = a.shape  
        b_batch, b_channel, b_height, b_width = b.shape
...
class Generator(nn.Module):
    def __init__(self,
                 ...
                 final_channel=2,  # Output channel size, for rgb always 3
        ...
        self.net_level_max_ = len(self.channel_list) - 1  # minus 1 in order to exclude last rgb layer
        ...
        self.net_level_ = self.net_level_max_ - 1  # set default net level as max level
    ...
    def _create_block(self, in_channels, out_channels, block_type):
        ...
        if block_type in ["First", "UpSample"]:
            if block_type == "First":
                ...
                block_cache.append(nn.Conv2d(self.latent_size_, out_channels,
                                             kernel_size=3, stride=1, padding=1, bias=False))
            if block_type == "UpSample" or block_type == 'final':
...
class Discriminator(nn.Module):
    def __init__(self,
        ...
        self.resolution_ = 2 ** image_pyramid_  # correct resolution
        self.channel_list = channel_list
        self.net_level_max_ = len(self.channel_list) - 1  # minus 1 in order to exclude first rgb layer
        self.lod_layers_ = nn.ModuleList()  # layer blocks exclude to_rgb layer
        self.rgb_layers_ = nn.ModuleList()  # rgb layers each correspond to specific level.
    ...
    def forward(self, x):
        ...
        if self.net_status_ == "stable":
            ...
            x_pre_cache = F.avg_pool2d(x, kernel_size=2, stride=2, padding=0, ceil_mode=False, count_include_pad=False)
            x_pre_cache = self.rgb_layers_[pre_input_level](x_pre_cache)
            x_cur_cache = self.rgb_layers_[cur_input_level](x)
            x_cur_cache = self.lod_layers_[cur_input_level](x_cur_cache)
            x = x_pre_cache * pre_weight + x_cur_cache * cur_weight
    ...
    def _construct_by_level(self, cursor):
        ...
        if self.channel_list is not None:  # specifies the number of convolutional filters for each layer
            in_channels=self.channel_list[in_level]
            out_channels=self.channel_list[out_level]
            print("Cursor",cursor,in_channels,out_channels)
        else:  # if not specified, recursion by the number of layers
            in_channels, out_channels = map(self._get_channel_by_stage, (in_level, out_level))  
        ...
        if block_type == "Minibatch":  # last layer
            self.discriminator_classifier= nn.Linear(self.channel_list[0]*16*16, 1)  # n_chan*width*height
            # self.discriminator_classifier = nn.Conv2d(self.channel_list[0], 1, kernel_size=3, stride=1, padding=1)  # patchGAN
